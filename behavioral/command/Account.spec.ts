...
test('Deve criar uma conta bancária', () => {
  const account = new Account()
  const balance = account.getBalance()
  expect(balance).toBe(0)
})

test('Deve creditar numa conta bancária', () => {
  const account = new Account()
  account.credit(1000)
  const balance = account.getBalance()
...
test('Deve creditar numa conta bancária via comando', () => {
  const account = new Account()
...
test('Deve debitar de uma conta bancária via comando', () => {
  const account = new Account()
  const creditCommand = new CreditCommand(account, 1000)
  creditCommand.execute()
